...
This repository tracks my study of UNIX internals, the C programming language, and the underlying computing model used by C and Linux to aind in my security research.
...
A series of standalone UNIX projects designed to interact directly with the Linux kernel.

* **Arena Allocator:** A custom linear memory allocator using the `mmap` syscall. Focuses on pointer arithmetic, memory alignment, and raw kernel memory requests.
...
Each project directory contains its own instructions or Makefile. Development is done natively on a Linux environment using standard GCC/Clang toolchains and debugging tools like GDB and Valgrind.
